...
namespace Inventarverwaltung_iad_asp
{
    public partial class WebForm1 : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
              //  SqlCon.checkLogin("Meyer", "*****");
                ...
                {
                    SqlCon.sqlConnection.Close();
                    Response.Redirect("~/Inventarverwaltung/Login.aspx", true);
                }
            }
            ...
            try
            {
                ...
                if (objekt.SelectedValue != "Computer") {
                    ...
                    string cpuval = auswahlcpu.SelectedValue, cpu = "\'"; cpu += cpuval; cpu += "\'";
                    if (auswahlcpu.SelectedValue.ToString() == "''") cpu = "''";
                    string grafikval = auswahlgrafik.SelectedValue, grafik = "\'"; grafik += grafikval; grafik += "\'";
                    if (auswahlgrafik.SelectedValue.ToString() == "''") grafik = "''";

                    SqlCon.command = new SqlCommand("exec dbo.getComputer " + orte.SelectedValue + "," + auswahlraum.SelectedValue + "," + cpu +
                        "," + auswahlram.SelectedValue + "," + auswahlhdd.SelectedValue + "," + auswahlssd.SelectedValue + "," + grafik, SqlCon.sqlConnection);
                }
        ...
        protected void qrbut_Click(object sender, EventArgs e)
        {

            string s = Server.MapPath("/Inventarverwaltung/qr");
            var file = new FileInfo(s + ".pdf");
            file.Delete();
            file = new FileInfo(s + "/qr.pdf");
            file.CopyTo(s + ".pdf");
            try
            {
                SqlCon.sqlConnection.Open();
                SqlCon.command = new SqlCommand("exec dbo.getInventar " + objekt.SelectedValue + "," + orte.SelectedValue + ","
                + auswahlraum.SelectedValue, SqlCon.sqlConnection);
                int count = 0, x = 10, y = 10;
                SqlCon.reader = SqlCon.command.ExecuteReader();
                while (SqlCon.reader.Read())
                {
                    var test = QRCodeWriter.CreateQrCode(SqlCon.reader["InventarId"].ToString(), 70, QRCodeWriter.QrErrorCorrectionLevel.Medium).SaveAsPng(s + ".png");

                    if (objekt.SelectedItem.ToString() == "Computer") {
                        test.AddAnnotationTextBelowBarcode(SqlCon.reader["Computername"].ToString()+"\nPos:"+ SqlCon.reader["Position"].ToString()+"\n-------------", 5);
                    }
                    else
                    {
                        test.AddAnnotationTextBelowBarcode(objekt.SelectedItem.ToString() + " " + SqlCon.reader[objekt.SelectedItem.ToString() + "Id"].ToString() + "\nPos:" + SqlCon.reader["Position"].ToString() + "\n-------------", 5);
                    }
                    test.StampToExistingPdfPage(s + ".pdf", x, y);
                    x += 75;count++;
                    if (count % 7 == 0)
                    {
                        y += 100;x = 10;
            ...
            Response.Redirect("~/Inventarverwaltung/qr.pdf", true);
        }
